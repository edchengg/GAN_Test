...
class GAN(object):
    def __init__(self, args):
        ...
        self.D = Discriminator(input_channel=data.shape[1], output_dim=1, input_size=self.input_size)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for idx, (x, _) in enumerate(self.data_loader):
                ...
                D_real = self.D(x)
                D_real_loss = self.BCE_loss(D_real, self.real_label)

                x_fake = self.G(z)
                D_fake = self.D(x_fake)
                D_fake_loss = self.BCE_loss(D_fake, self.fake_label)
                ...
                x_fake = self.G(z)
                D_fake = self.D(x_fake)
                G_loss = self.BCE_loss(D_fake, self.real_label)
                G_loss.backward()
